document.addEventListener('DOMContentLoaded', function() {
    ...
    const floorOpeningInput = document.getElementById('floorOpening');
    ...
    const paramRanges = {
        ...
        blondelValue: { min: 59, max: 65, recommended: '61-64', unit: 'cm' },
    ...
    // 监听所有输入元素的变化
    [totalHeightInput, riserHeightInput, treadDepthInput, totalDepthInput,
     treadThicknessInput, structureTypeSelect, headroomInput, floorOpeningInput,
     blondelTypeSelect].forEach(input => {
    ...
    function calculateStairs() {
        ...
        const headroom = (parseFloat(headroomInput.value) || 200) * 10;
        const floorOpening = (parseFloat(floorOpeningInput.value) || 220) * 10;
        ...
        // 净空限制检查
        const minHeadroom = headroom;
        let headroomViolation = false;
        
        // 简化的净空检查：确保楼梯任何位置的净空都满足要求
        const maxStairHeight = actualRiserHeight * numberOfSteps;
        if (maxStairHeight + minHeadroom > floorOpening) {
            headroomViolation = true;
        }
    ...
    function displayResults(results) {
        ...
        const structureType = structureTypeSelect.value;
        const headroomViolation = results.headroomViolation;
        const totalDepthAdjusted = results.totalDepthAdjusted;

        const resultItems = [
            { label: '台阶数量', value: `${results.numberOfSteps} 级`, range: '根据总高度自动计算' },
            { label: '实际单层高度', value: `${(results.actualRiserHeight/10).toFixed(1)} cm`, range: `推荐 ${paramRanges.riserHeight.recommended}` },
            { label: '踏板深度', value: `${(results.treadDepth/10).toFixed(1)} cm`, range: `推荐 ${paramRanges.treadDepth.recommended}` },
            { label: '总高度', value: `${(parseFloat(totalHeightInput.value) || 200).toFixed(1)} cm`, range: '用户输入的总高度' },
            { label: '总深度', value: `${(results.actualTotalDepth/10).toFixed(1)} cm${totalDepthAdjusted ? ' (已根据限制调整)' : ''}`, range: '根据台阶数量计算' },
            { label: '斜梁长度', value: `${(results.stringerLength/10).toFixed(1)} cm`, range: '楼梯斜向支撑梁的长度，计算公式：√(总高度² + 总深度²)' },
            { label: '楼梯角度', value: `${stairAngle.toFixed(1)}°`, range: `推荐 ${paramRanges.stairAngle.recommended}` }
        ];
        ...
        validations.push(`Blondel值为 ${results.blondelValue.toFixed(0)}mm，${blondelStatus}`);
        ...
        // 显示净空限制信息
        if (headroomViolation) {
            validations.push(`⚠ 净空高度可能不足，请检查`);
        }
